...
void draw_object(Object *o, BITMAP *scr, int ox, int oy) {
    if(o->shape) {
        int x, y, xalt, yalt;
        ...
        for(int i = 3; o->shape[i].x != 0 || o->shape[i].y != 0; ++i) {
            ...
            int tx = ((int)(x-ox+SCREEN_W*2.5))/(2*SCREEN_W);
            int ty = ((int)(y-oy+SCREEN_H*2.5))/(2*SCREEN_H);
            line(scr, (x-ox+SCREEN_W*2.5)-(2*SCREEN_W)*tx,
                      (y-oy+SCREEN_H*2.5)-(2*SCREEN_H)*ty,
                      (xalt-ox+SCREEN_W*2.5)-(2*SCREEN_W)*tx,
                      (yalt-oy+SCREEN_H*2.5)-(2*SCREEN_H)*ty, makecol(255,255,255));
            xalt = x;
            yalt = y;
        }
    }
